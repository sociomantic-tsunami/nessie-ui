...
}) =>
{
    ...
    const realSelectedValues = selectedValues.length ?
    selectedValues : selectedValuesState;

    let items = children ?
        Children.toArray( children ) : buildCheckboxesFromValues( values, onChange, selectedValues.length ? null : selectedValuesState, setSelectedValuesState );

    items = items.map( item => React.cloneElement( item, {
        isChecked : realSelectedValues.includes( item.props.value ),
    } ) );
